...
async function runPhase7TestSuite() {
  ...
  try {
    ...
    assert('Manifest Name Defined', Boolean(manifest.name && manifest.name.includes('MineralHub')));
    assert('Manifest Short Name Valid', manifest.short_name === 'MineralHub');
    ...
    assert('Static manifest.json Exists', fs.existsSync(manifestJsonPath) && fs.statSync(manifestJsonPath).size > 0);
  } catch (err: any) {
  ...
  try {
    ...
    const verifiedOrder = await updateOrderStatus(newOrder.id, 'PAID', 'Pembayaran diverifikasi oleh admin');
    assert('Admin Approves Payment -> Status PAID', verifiedOrder.status === 'PAID');
