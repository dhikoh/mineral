...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between bg-slate-900 border-b border-slate-800 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-white">Adably Admin</span>
        </div>
        <button
          ...
          <aside className="relative flex flex-col w-64 bg-slate-900 border-r border-slate-800 h-full">
            ...
            <div className="border-t border-slate-800 px-3 py-3">
              <button
                onClick={handleLogout}
                disabled={loggingOut}
                className="w-full flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2 text-xs font-medium text-slate-300 hover:bg-rose-500/10 hover:border-rose-500/40 hover:text-rose-300 transition-colors"
              >
                <LogOut className="h-3.5 w-3.5" />
                {loggingOut ? 'Keluar...' : 'Keluar'}
              </button>
      ...
      <main className="flex-1 lg:ml-56 xl:ml-64 pt-0 lg:pt-0">
        <div className="lg:hidden h-[53px]" /> {/* Spacer for mobile header */}
        {children}
      </main>
    </div>
  );
